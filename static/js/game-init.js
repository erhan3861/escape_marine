 ...
 *   1. base-injector.js   → window.isLocalEnvironment
 *   2. media-preloads.js  → audioPreloads, imagePreloads
 *   3. game-engine-utils.js → preloadMedia, updateTextMessage, loadScript
 *   4. window.gameObject  → HTML içinde tanımlanmış olmalı
 * -------------------------------------------------------
 */

/**
 * Sabit / statik Türkçe etiket çevirileri.
 * Genially motorunun ürettiği İngilizce metinleri Türkçeye çevirir.
 * Soru içeriğiyle ilgisi yoktur; UI etiketleri + talimatlar.
 */
...
/**
 * UUID → gameObject anahtarı eşlemesini data.json üzerine uygular.
 * Her Genially elementi benzersiz UUID ile tanımlanır; bu fonksiyon
 * o UUID'leri window.gameObject içindeki placeholder değerleriyle eşler.
 *
 * @param {Object} data       - data.json'dan yüklenen ham oyun verisi
 * @param {Object} gameObject - window.gameObject (AI tarafından doldurulmuş)
 */
function applyGameObjectMapping(data, gameObject) {

  // ── Metin Bloğu Eşlemeleri ───────────────────────────────────────────────
  const textMapping = {
    // Karşılama ekranı
    ...
    // Eşleştirme aktivite başlıkları ve öğeleri
    ...
    // Görsel sorular – başlık + açıklama çiftleri
...
async function init() {
  ...
  try {
    ...
    // Ortama göre data.json yolunu belirle
    ...
    // Soru ve metin eşlemelerini uygula
    ...
    // Oyun motorlarını sırayla yükle
